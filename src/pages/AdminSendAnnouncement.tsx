...
const AdminSendAnnouncement = () => {
  ...
  const handleSend = () => {
    if (!title.trim()) {
      toast({ title: "⚠️ Enter a title!", variant: "destructive" });
      return;
    }
    if (!content.trim()) {
      toast({ title: "⚠️ Enter the announcement content!", variant: "destructive" });
      return;
    }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground font-body">Send notices & alerts to specific classes. All students in selected classes will see this.</p>
      </div>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              className="rounded-xl font-body mt-1 h-12"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={priority} onValueChange={(v) => setPriority(v as "high" | "medium" | "low")}>
              <SelectTrigger className="rounded-xl font-body mt-1">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      >
        {sent ? (
          ...
          <>
            <Send className="w-6 h-6 mr-2" /> Send to{" "}
